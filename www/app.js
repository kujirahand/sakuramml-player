...
const mmlLog = document.getElementById('mml-log');

let compileLogText = '';
let lastLoggedLyricIndex = -1;
...
function compileAndPlayMml(mml) {
  ...
  setTimeout(() => {
    try {
      ...
      console.error(e);
      if (mmlLog.value === '') mmlLog.value = 'コンパイルエラー: ' + e;
    }
  }, 10);
}
...
function loadMidiBytes(bytes, title) {
  ...
  try {
    ...
    texts = data.texts || [];
    resetLyricLog();
    console.log('Parsed text events:', texts);
...
function drawTexts(W, H, ct) {
  const startSec = scrollX / pps;
  const endSec = (scrollX + W - PIANO_W) / pps;
  const lyricY = RULER_H + 22;
  const metaY = RULER_H + 4;
  ...
  for (const t of texts) {
    // 0x01 = Text, 0x05 = Lyric, 0x06 = Marker
    if (t.type !== 1 && t.type !== 5 && t.type !== 6) continue;
    if (t.time < startSec || t.time > endSec) continue;

    const x = PIANO_W + t.time * pps - scrollX;
    if (x < PIANO_W) continue;

    if (t.type === 5) {
      ctx2d.fillStyle = t.time <= ct ? 'rgba(255, 216, 107, 0.98)' : 'rgba(255, 216, 107, 0.78)';
      ctx2d.fillText(t.text, x + 4, lyricY);
    } else {
      ctx2d.fillStyle = 'rgba(255, 255, 255, 0.9)';
      ctx2d.fillText(t.text, x + 4, metaY);
    }
  }
}
...
function setCompileLog(log) {
  ...
  compileLogText = normalized.length > 0 ? normalized : 'コンパイル成功！';
  lastLoggedLyricIndex = -1;
  mmlLog.value = compileLogText;

  if (normalized.includes('ERROR') || normalized.includes('Error')) {
    mmlLog.classList.remove('success');
  } else {
    mmlLog.classList.add('success');
  }
}

function resetLyricLog() {
  lastLoggedLyricIndex = -1;
  mmlLog.value = compileLogText;
}

function syncLyricLog(currentTime) {
  if (!mmlLog) return;

  const lyricEvents = texts.filter(t => t.type === 5);
  if (lyricEvents.length === 0) {
    if (mmlLog.value !== compileLogText) {
      mmlLog.value = compileLogText;
    }
    return;
  }
  ...
  for (let i = 0; i < lyricEvents.length; i++) {
    if (lyricEvents[i].time <= currentTime) {
      nextIndex = i;
    } else {
      break;
    }
  ...
  if (nextIndex === lastLoggedLyricIndex && mmlLog.value.length > 0) {
    return;
  }

  const lyricLines = lyricEvents
    .slice(0, nextIndex + 1)
    .map(event => event.text)
    .filter(text => typeof text === 'string' && text.length > 0);

  mmlLog.value = lyricLines.length > 0
    ? `${compileLogText}\n\n[Lyric]\n${lyricLines.join('\n')}`
    : compileLogText;
  mmlLog.scrollTop = mmlLog.scrollHeight;
  lastLoggedLyricIndex = nextIndex;
}
